...
// Resolved here rather than inline below, where the `config` argument of getDBConnection shadows
// the module-level wild-config this reads from.
let mongoClientOptions = () =>
    Object.assign(
        ...
        (config.dbs && config.dbs.mongoOptions) || {}
    );
...
let getDBConnection = (main, config, callback) => {
    ...
    MongoClient.connect(config, mongoClientOptions(), (err, db) => {
        if (err) {
            return callback(err);
        }
