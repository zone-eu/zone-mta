            ...
            getKeyfiles(() => {
                let defaultLogger = nodemailerLogger.create((level, ...args) => {
                    switch (level) {
                        case 'trace':
                        case 'debug':
                            return log.silly(this.options.name, ...args.slice(1));
                        case 'warn':
                            return log.warn(this.options.name, ...args.slice(1));
                        case 'error':
                        case 'fatal':
                            return log.error(this.options.name, ...args.slice(1));
                    }

                    return log.verbose(this.options.name, ...args.slice(1));
                });
