...
class SendingZone {
    ...
    constructor(name, zone, queue) {
        ...
        this.blockDomains = [].concat(zone.blockDomains || config.dns.blockDomains);

        // copy properties from the config object
        ['logger', 'logLevel', 'resolveIp', 'ignoreIPv6', 'port', 'host', 'auth', 'ignoreTLS', 'requireTLS', 'secure', 'authMethod', 'poolHash'].forEach(key => {
            if (key in zone) {
                this[key] = zone[key];
            }
        });
    ...
    getAddress(delivery, useIPv6, domainConfig) {
        let key;

        domainConfig = domainConfig || this.domainConfig(delivery.domain);
        ...
        if (domainConfig.disabledAddresses && domainConfig.disabledAddresses.length) {
            pool = pool.filter(entry => !domainConfig.disabledAddresses.includes(entry.address));
            if (!pool.length) {
    ...
    spawnSender() {
        ...
        let child = child_process.fork(__dirname + '/../sender.js', [this.name, childId]);
        let pid = child.pid;
    ...
    removeFromBlacklist(data) {
        let domainConfig = this.domainConfig(data.domain);
        if (domainConfig && Array.isArray(domainConfig.disabledAddresses) && domainConfig.disabledAddresses.includes(data.address)) {
            for (let i = 0, len = domainConfig.disabledAddresses.length; i < len; i++) {
                if (domainConfig.disabledAddresses[i] === data.address) {
                    domainConfig.disabledAddresses.splice(i, 1);
    ...
    deferDelivery(delivery, ttl, responseData, callback) {
        ...
        if (responseData.category === 'blacklist' && responseData.address && delivery.domain) {
            // block IP for domain
            let domainConfig = this.domainConfig(delivery.domain);
            if (!domainConfig.disabledAddresses || !domainConfig.disabledAddresses.includes(responseData.address)) {
                ...
                if (!domainConfig.disabledAddresses) {
                    domainConfig.disabledAddresses = [];
                } else if (!Array.isArray(domainConfig.disabledAddresses)) {
                    domainConfig.disabledAddresses = [].concat(domainConfig.disabledAddresses || []);
                }
                domainConfig.disabledAddresses.push(responseData.address);

                // blacklist IP for MX for next 6 hours
                this.queue.cache.set('blacklist:' + delivery.domain + ':' + responseData.address, {
                    id: delivery.id,
                    seq: delivery.seq,
                    ...
                    created: Date.now()
                }, config.blacklist.ttl, data => this.removeFromBlacklist(data));
            }
        }
