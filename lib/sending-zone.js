...
class SendingZone {
            ...
            if ((!item.name || item.resolveName) && net.isIP(item.address)) {
                ...
                dns.reverse(addr, (err, hostnames) => {
                    if (err && err.code !== 'ENOTFOUND') {
                        ...
                        emitGelf({
                            short_message: `${gelfCode('DNS_REVERSE_FAILED')} Failed to reverse IP address`,
    ...
    // Tell every sender child of this zone to shut down, see lib/child-shutdown.js
    close() {
        childShutdown.notifyChildren(this.children, 'Child/' + this.name);
    }
    ...
    spawnSender(callback) {
        ...
        let pid = child.pid;
        // Track the process from fork(), not only after its startup handshake. Otherwise a
        // shutdown during startup neither notifies this child nor waits for it to exit.
        ...
        let timer = setTimeout(
            () => {
                ...
                emitGelf({
                    short_message: `${gelfCode('SENDER_START_TIMEOUT')} Sender process startup timeout`,
                    _logger: 'Child/' + this.name + '/' + pid,
                    _zone: this.name,
                    _child_id: childId,
                    _pid: pid,
                    full_message: 'Sender process startup timeout',
                    _error: 'Sender process startup timeout'
                });
                child.kill('SIGTERM');
            },
            2 * 60 * 1000
        ...
        child.once('close', (code, signal) => {
            ...
            if (this.queue && !this.queue.closing) {
                ...
                emitGelf({
                    short_message: `${gelfCode('SENDER_EXITED')} Sender process exited unexpectedly`,
                    _logger: 'Child/' + this.name + '/' + pid,
                    _zone: this.name,
                    _child_id: childId,
                    _pid: pid,
                    _exit_code: code,
                    _signal: signal,
                    full_message: 'Sender process exited unexpectedly',
                    _error: 'Sender process exited unexpectedly'
                });
    ...
    speedometer(ref, next) {
        ...
        this.throttling.timers.set(ref, curTime + nextCheck);
...
// Number of sender processes that are still running, used to wait out a graceful shutdown.
module.exports.countChildren = () => {
    let count = 0;
    sendingZonelist.forEach(zone => (count += zone.children.size));
    return count;
};
