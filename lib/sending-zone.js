...
class SendingZone {
    ...
    constructor(name, zone, domainConfig, queue) {
        ...
        ['logger', 'logLevel', 'resolveIp', 'ignoreIPv6', 'preferIPv6', 'port', 'host', 'auth', 'ignoreTLS', 'requireTLS', 'secure', 'authMethod', 'poolHash', 'disabled'].forEach(key => {
            if (key in zone) {
                this[key] = zone[key];
            }
        });
    ...
    spawnSender() {
        if (!this.queue || this.spawned >= this.processes || this.queue.closing || this.disabled) {
            return false;
        }
