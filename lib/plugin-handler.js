...
class PluginInstance {
    ...
    drop(id, description, messageInfo, responseText) {
        ...
        return {
            name: 'SMTPResponse',
            message: responseText || ('Message queued as ' + id)
        };
    }

    reject(id, description, messageInfo, responseText) {
        description = (description || '').toString().trim();
        if (messageInfo && typeof messageInfo.format === 'function') {
            messageInfo = messageInfo.format();
        }
            ...
            try {
                ...
                return plugin.module.init(new PluginInstance(this, plugin), err => {
                    ...
                    if (err) {
                        ...
                        this.logger.verbose('Plugins', 'Initialized %s from <%s> [load time %sms]', plugin.title, path.relative(process.cwd(), plugin.path), loadTime);
    ...
    preparePlugins(pluginData) {
        return Object.keys(pluginData || {})
            .map(key => {
                ...
                let pluginPath = path.resolve(/^[\.\/]*core\//.test(key) ? this.corePluginsPath : this.pluginsPath, key);
