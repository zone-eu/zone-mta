...
class PluginInstance {
    ...
    getApiServer() {
        return this.manager.apiServer && this.manager.apiServer.server;
    }
}
...
class PluginHandler {
    constructor(options) {
        ...
        this.context = options.context || 'feeder';
        ...
        let loadNext = () => {
            ...
            try {
                ...
                if (!plugin.module || typeof plugin.module.init !== 'function') {
                    ...
                    this.logger.info('Plugins', 'Plugin %s from <%s> does not have an init method [load time %sms]', plugin.title, path.relative(process.cwd(), plugin.path), loadTime);
                    // not much to do here
                    return loadNext();
                }
                ...
                return plugin.module.init(new PluginInstance(this, plugin), err => {
                    let loadTime = Date.now() - loadStartTime;
                    if (err) {
                        this.logger.error('Plugins', 'Failed loading plugin %s from <%s>: %s [load time %sms]', plugin.title, path.relative(process.cwd(), plugin.path), err.message, loadTime);
                    } else {
                        this.logger.info('Plugins', 'Initialized %s from <%s> [load time %sms]', plugin.title, path.relative(process.cwd(), plugin.path), loadTime);
    ...
    preparePlugins(pluginData) {
        return Object.keys(pluginData || {})
            .map(key => {
                ...
                // Only load plugins with correct context. If context is not set then default to "feeder"
                let allowedContext = [].concat(pluginConfig.enabled || 'feeder').map(context => {
