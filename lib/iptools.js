...
module.exports.resolveIp = (domain, options, callback) => {
    ...
    let ipv6err, ipv4err;
    let filterResolvedList = entry => filterLocalAddresses(entry, options.blockDomains || config.dns.blockDomains);
...
function filterLocalAddresses(entry, blockDomains) {
    if (!entry || blockDomains.includes(entry)) {
        return false;
    }
    if (config.dns.blockLocalAddresses) {
        if (isLocal(entry)) {
            return false;
        }
