...
module.exports.resolveIp = (domain, options, callback) => {
    ...
    let ipv6err, ipv4err;
    let filterResolvedList = entry => filterLocalAddresses(entry, 'blockDomains' in options ? options.blockDomains : config.dns.blockDomains, 'blockLocalAddresses' in options ? options.blockLocalAddresses : config.dns.blockLocalAddresses);
    ...
    let done = () => {
        ...
        list = list.filter(filterResolvedList);
...
module.exports.resolveMx = (domain, options, callback) => {
    ...
    dns.resolveMx(domain, (err, list) => {
        ...
        list = list.filter(entry => filterResolvedList(entry.exchange)).sort((a, b) => a.priority - b.priority);
...
function filterLocalAddresses(entry, blockDomains, blockLocalAddresses) {
    if (!entry || (blockDomains && blockDomains.includes(entry))) {
        return false;
    }
    if (blockLocalAddresses) {
        ...
        if (net.isIP(entry) && ['unspecified', 'loopback', 'private', 'broadcast'].includes(ipaddr.parse(entry).range())) {
            return false;
        }
    }
    return true;
