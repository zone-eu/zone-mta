...
const dnsCachingEnabled = !!config.dns.caching;
...
const resolveRecords = (domain, type) =>
...
/**
 * Adds ZoneMTA's cache-backed resolver to mx-connect 2.x without replacing a resolver
 * supplied for this delivery. mx-connect otherwise uses dns.promises directly, which
 * bypasses the callback methods patched by dnscache above.
 *
 * @param {Object} dnsOptions mx-connect DNS options
 * @returns {Object} DNS options for mx-connect
 */
module.exports.getMxConnectDnsOptions = dnsOptions => {
    const options = dnsOptions || {};
    const hasCustomResolver = ['resolveRecords', 'resolve'].some(key => options[key] !== undefined && options[key] !== null);

    if (!dnsCachingEnabled || hasCustomResolver) {
        return options;
    }

    return Object.assign({}, options, { resolveRecords });
};
