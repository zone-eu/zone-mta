...
class Sender extends EventEmitter {
    ...
    sendBounceMessage(delivery, bounce, smtpResponse) {
        ...
        if (/\ball\b/i.test(xAutoResponseSuppress)) {
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SKIPBOUNCE Skip bounce to %s due to presence of %s header (%s)', delivery.id, delivery.seq, delivery.from, 'X-Auto-Response-Suppress', JSON.stringify(xAutoResponseSuppress));
            return;
        }

        let autoSubmitted = delivery.headers.getFirst('Auto-Submitted');
        if (/\bauto\-(generated|replied)\b/i.test(autoSubmitted)) {
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SKIPBOUNCE Skip bounce to %s due to presence of %s header (%s)', delivery.id, delivery.seq, delivery.from, 'Auto-Submitted', JSON.stringify(autoSubmitted));
            return;
        }

        if (/^mailer\-daemon@/i.test(delivery.from) || !delivery.from) {
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SKIPBOUNCE Skip bounce to %s due to MAIL FROM address (%s)', delivery.id, delivery.seq, delivery.from, 'Auto-Submitted', JSON.stringify(delivery.from || ''));
            return;
        }

        if (/^mailer\-daemon@/i.test(delivery.parsedEnvelope.from)) {
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SKIPBOUNCE Skip bounce to %s due to From header (%s)', delivery.id, delivery.seq, delivery.from, 'Auto-Submitted', JSON.stringify(delivery.parsedEnvelope.from || ''));
            return;
        }
