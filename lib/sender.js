            ...
            plugins.handler.runHooks('sender:fetch', [delivery], err => {
                ...
                this.zone.speedometer(this.ref, () => {
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        // Custom DKIM Hook for HTTP
                        if (connection.http) {
                            plugins.handler.runHooks('http:dkim', [delivery, connection], err => {
                                if (err) {
                                    connectionDone = true;
                                    connection.close();
                                    return handleError(delivery, connection, err);
                                ...
                                if (!connection.http) {
                                    ...
                                    messageStream.once('end', () => {
                                        ...
                                        if(!delivery.send) {
                                            ...
                                            delivery.send(messageContent, (err, info) => {
                                                if (err) {
                                                    err.protocol = 'http';
                                                    err.httpUrl = connection.targetUrl;
                                                    err.httpResponse = info.statusCode || '-';

                                                    err.responseCode = info.statusCode || 550;
                                                    return next(err);
                                                }
                                                ...
                                                next(null, {
                                                    ...
                                                    protocol: 'http',
                                                    httpUrl: connection.targetUrl,
                                                    httpResponse: info.statusCode,
                                                    response:
                                                        'Message uploaded. Received ' +
                                                        (info.body ? info.body.length : 0) +
                                                        ' B response with status code ' +
                                                        info.statusCode +
                                                        '.'
                                                });
