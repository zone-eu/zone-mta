                ...
                iptools.resolveIp(exchange.exchange, this.zone, (err, ipList) => {
                    ...
                    let tryConnectIP = retryConnection => {
                        ...
                        plugins.handler.runHooks('sender:connect', [delivery, options], err => {
                            ...
                            connection.connect(() => {
                                log.verbose('Sender/' + this.zone.name + '/' + process.pid, '%s.%s INITIALIZED (mx=%s mta=%s) (%s)', delivery.id, delivery.seq, options.host, options.localAddress, returned ? 'returned' : 'done');
                                if (returned) {
                                    return;
                                }
