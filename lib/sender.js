...
class Sender extends EventEmitter {
                ...
                this.zone.speedometer(this.ref, () => { // check throttling speed
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        if (!connection.http) {
                            ...
                            connection.once('end', () => {
                                log.info(this.logName, '%s.%s SMTPCLOSE [%s] Connection closed response="%s"', delivery.id, delivery.seq, connection.id, connection.lastServerResponse || '');
                                    ...
                                    messageStream.once('end', () => {
                                        ...
                                        request.post({
                                            ...
                                            if (httpResponse.statusCode > 299 || httpResponse.statusCode < 200) {
                                                err = new Error(httpResponse.statusCode + ' Upload failed. Received ' + (body ? body.length : 0) + ' B response');
                                                err.responseCode = httpResponse && httpResponse.statusCode || 550;
                                                return next(err);
                                            }
                                            ...
                                            next(null, {
                                                ...
                                                rejected: [],
                                                response: 'Message uploaded. Received ' + (body ? body.length : 0) + ' B response with status code ' + httpResponse.statusCode + '.'
                                            });
                                        });
                                    });
                                }
        ...
        let tryConnect = () => {
            ...
            connect(delivery, (err, mx) => {
                ...
                let _onError = err => {
                    ...
                    if ((err.code === 'ETLS' || /SSL23_GET_SERVER_HELLO|\/deps\/openssl/.test(err.message) || err.code === 'ECONNRESET') && !this.tlsDisabled.has(mx.host)) {
                        // STARTTLS failed, try again, this time without encryption
                        log.info(this.logName, '%s.%s ERRCONNECT [%s] Failed to connect to %s[%s] using STARTTLS, proceeding with plaintext', delivery.id, delivery.seq, connId, mx.hostname, mx.host);
                ...
                connection.once('end', () => {
                    log.info(this.logName, '%s.%s SMTPCLOSE [%s] Closed prematurely response="%s" connected=%ss.', delivery.id, delivery.seq, connId, connection.lastServerResponse || '', (Date.now() - startTime) / 1000);
                    connection.connected = false;
                    if (returned) {
                        return;
                    }
    ...
    sendBounceMessage(delivery, bounce, smtpResponse) {
        ...
        if (/^multipart\/report\b/i.test(contentType)) {
            log.info(this.logName, '%s.%s SKIPBOUNCE Skip bounce to %s due to header (%s=%s)', delivery.id, delivery.seq, delivery.from || '<>', 'Content-Type', 'multipart/report');
            return;
        }
