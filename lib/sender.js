...
const connect = require('./sender/connect');
...
class Sender extends EventEmitter {
    constructor(clientId, connectionId, zone, sendCommand, queue) {
        ...
        log.info(this.logName, 'Created sender instance');
    ...
    close() {
        if (!this.closing) {
            log.info(this.logName, 'Closing sender instance');
            this.closing = true;
        }
    }
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            let send = next => {
                                ...
                                if (!connection.http) {
                                    ...
                                    messageStream.once('end', () => {
                                        ...
                                        let headerValues = {
                                            from: addressTools.parseAddressList(delivery.headers, 'from', true).shift() || false,
                                            to: addressTools.parseAddressList(delivery.headers, 'to', true),
                                            cc: addressTools.parseAddressList(delivery.headers, 'cc', true),
                                            bcc: addressTools.parseAddressList(delivery.headers, 'bcc', true),
                                            replyTo: addressTools.parseAddressList(delivery.headers, 'reply-to', true).shift() || false,
                                            sender: addressTools.parseAddressList(delivery.headers, 'sender', true).shift() || false,
                                            date: delivery.headers.getFirst('Date'),
                                            subject
                                        };

                                        let headers = {};
                                        Object.keys(headerValues)
                                            .map(key => {
                                                if (!headerValues[key] || !headerValues[key].length) {
                                                    return false;
                                                }
                                                return { key, value: headerValues[key] };
                                            })
                                        ...
                                        let formData = {
                                            ...
                                            'mail_to_http[from]': delivery.from,
                                            'mail_to_http[rcpt]': delivery.recipient,
                                            'mail_to_http[real_length]': messageContent.length,
                                            'mail_to_http[body]': messageContent.length <= 500 * 1024 ? messageContent : messageContent.slice(0, 500 * 1024),
                                            message: {
                            ...
                            send((err, info) => {
                                ...
                                log.info(
                                    this.logName,
                                    '%s.%s ACCEPTED from=%s to=%s src=%s dst=%s id=%s (%s)',
                                    delivery.id,
                                    delivery.seq,
                                    (delivery.from || '') + (delivery.from !== envelopeFrom ? '[' + envelopeFrom + ']' : '') || '<>',
                                    envelopeRecipient || '',
                                    (connection && connection.options.localAddress) || '',
                                ...
                                this.releaseDelivery(delivery, (err, released) => {
                                    if (err) {
                                        log.error(this.logName, '%s.%s Can\'t get message acknowledged. %s', delivery.id, delivery.seq, err.message);
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < ttlMinutes.length) {
            ...
            log.info(
                this.logName,
                '%s.%s DEFERRED[%s] from=%s to=%s src=%s dst=%s id=%s (%s)',
                delivery.id,
                delivery.seq,
                bounce.category,
                (delivery.from || '') + (delivery.from !== envelopeFrom ? '[' + envelopeFrom + ']' : '') || '<>',
            ...
            log.info(
                this.logName,
                '%s.%s REJECTED[%s] from=%s to=%s src=%s dst=%s id=%s (%s)',
                delivery.id,
                delivery.seq,
                bounce.category,
                (delivery.from || '') + (delivery.from !== envelopeFrom ? '[' + envelopeFrom + ']' : '') || '<>',
    ...
    getConnection(delivery, done) {
        ...
        let tryConnect = () => {
            ...
            connect(delivery, (err, mx) => {
                ...
                log.info(this.logName, '%s.%s CONNECTED domain=%s mx=%s[%s]', delivery.id, delivery.seq, delivery.domain, mx.hostname, mx.host);
                ...
                let _onError = err => {
                    ...
                    log.error(this.logName, '%s.%s ERRCONNECT [%s] Unexpected MX error. error="%s"', delivery.id, delivery.seq, connId, err.message);
                    return callback(err);
                };

                connection.once('error', err => {
                    log.error(this.logName, '%s.%s SMTPERR [%s] error="%s"', delivery.id, delivery.seq, connId, err.message);
                    _onError(err);
                });
                ...
                connection.once('end', () => {
                    log.info(
                        this.logName,
                        '%s.%s SMTPCLOSE [%s] Closed prematurely response="%s" connected=%ss.',
                        delivery.id,
                        delivery.seq,
                        connId,
                        connection.lastServerResponse || '',
