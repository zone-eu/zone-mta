                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            messageFetch.once('error', err => {
                                if (messageStream) {
                                    ...
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s UNEXPECTED [%s] message fetch: %s', delivery.id, delivery.seq, connection.id, JSON.stringify(String(err.stack)));
                                }
                            });
