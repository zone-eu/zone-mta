...
class Sender extends EventEmitter {
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            send((err, info) => {
                                ...
                                // release the connection, we don't need it anymore
                                ...
                                let connCachConfig = this.zone.connectionCache || { ttl: 5, reuseCount: 100 };
                                let connCacheTTL = connCachConfig.ttl || 5;
                                let connCacheMaxReuse = connCachConfig.reuseCount || 100;
                                if (!err && delivery.connectionKey && connection.usageCount < connCacheMaxReuse) {
                                    ...
                                    // delivery worked, now cache connection
                                    if (!connectionPool.has(delivery.connectionKey)) {
    ...
    getConnection(delivery, done) {
        ...
        // in case there is no cached connection create a new one
        let tryConnect = () => {
