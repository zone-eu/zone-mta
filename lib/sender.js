                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            send((err, info) => {
                                ...
                                let killConnection = (err, connection) => {
                                    // realease this connection, we don't need it anymore
                                    if (!connection || !connection.connected || connection._closing || connection._destroyed) {
                                        return;
                                    }
                                ...
                                if (!err && delivery.connectionKey && connection.usageCount < 100) {
                                    ...
                                    }, 5 * 1000); // keep connection alive max 5 seconds or new delivery is found
                                    killTimer.unref();
                                    ...
                                    if (!connectionPool.has(delivery.connectionKey)) {
                                        connectionPool.set(delivery.connectionKey, [conn]);
                                    } else {
                                        connectionPool.get(delivery.connectionKey).push(conn);
                                    }
                                } else {
        ...
        let tryConnect = () => {
            ...
            if (connectionPool.has(delivery.connectionKey)) {
                ...
                if (connList.length) {
                    ...
                    clearTimeout(conn.timer); // prevent closing socket
                    if (conn.connection && conn.connection.connected && !conn.connection._closing && !conn.connection._destroyed) {
                        ...
                        conn.connection.logtrail = logtrail; // reset logtrail
                        log.info(
                            this.logName,
