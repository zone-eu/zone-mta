            ...
            plugins.handler.runHooks('sender:fetch', [delivery], err => {
                ...
                this.zone.speedometer(this.ref, () => { // check throttling speed
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        connection.once('end', () => {
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s CLOSED [%s]', delivery.id, delivery.seq, connection.id);
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery], err => {
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SENDING [%s] (from=%s to=%s size=%s)', delivery.id, delivery.seq, connection.id, delivery.envelope.from, delivery.envelope.to, messageSize);
                        ...
                        plugins.handler.runHooks('sender:connect', [delivery, options], err => {
                            ...
                            let _onError = err => {
                                ...
                                if (!connected) {
                                    // try next host
                                    if (mxTry >= exchanges.length) {
                                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s ERRCONNECT [%s] %s[%s] for %s from %s (%s[%s]). "%s"', delivery.id, delivery.seq, connId, exchange.exchange, ip, domain, this.zone.name, zoneAddress.name, zoneAddress.address, err.message);
                                    }
                                    return tryConnectIP(err);
                                }
                            ...
                            connection.once('end', () => {
                                log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s CLOSED [%s]', delivery.id, delivery.seq, connId);
                                connection.connected = false;
                                if (returned) {
                                    return;
                                }
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s INITIALIZING [%s] (mx=%s mta=%s)', delivery.id, delivery.seq, connId, options.host, options.localAddress);
                            connection.connect(() => {
                                log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s INITIALIZED [%s] (mx=%s mta=%s)', delivery.id, delivery.seq, connId, options.host, options.localAddress);
                                if (returned) {
                                    return;
                                }
