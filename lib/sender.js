                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            connection.send({
                                ...
                                if (messageFetch.readable) {
                                    ...
                                    input.once('end', () => {
                                        input = null;
                                        log.verbose('Sender/' + this.zone.name + '/' + process.pid, '%s.%s UNCLOSED [%s] Socket was not closed url=%s readsize=%s', delivery.id, delivery.seq, connection.id, fetchUrl, readsize);
                                    });
                                }
