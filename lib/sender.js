...
class Sender extends EventEmitter {
    ...
    sendNext() {
        ...
        this.sendCommand('GET', (err, delivery) => {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s FETCHED for delivery (from=%s to=%s)', delivery.id, delivery.seq, delivery.from, delivery.to);
            ...
            this.zone.speedometer(this.ref, () => { // check throttling speed
                ...
                this.getConnection(delivery, (err, connection) => {
                    ...
                    log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s CONNECTED mx=%s[%s]', delivery.id, delivery.seq, connection.options.servername, connection.options.host);
                    ...
                    plugins.handler.runHooks('sender:headers', [delivery], err => {
                        ...
                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s SENDING (from=%s to=%s size=%s)', delivery.id, delivery.seq, delivery.envelope.from, delivery.envelope.to, messageSize);
                        ...
                        connection.send({
                            ...
                            this.releaseDelivery(delivery, err => {
                                if (err) {
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s Can\'t get message acknowledged', delivery.id, delivery.seq);
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, err.message);
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < maxDeferred) {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s DEFERRED[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.recipient, connection && connection.options.servername || delivery.domain, smtpResponse);

            return this.deferDelivery(delivery, ttl, smtpResponse, err => {
                if (err) {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s REJECTED[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.recipient, connection && connection.options.servername || delivery.domain, smtpResponse);
            ...
            let tryConnectMX = () => {
                ...
                iptools.resolveIp(exchange.exchange, this.zone, (err, ipList) => {
                    ...
                    let tryConnectIP = retryConnection => {
                        ...
                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s RESOLVED MX for %s as %s[%s]. Using %s (%s[%s]) to connect', delivery.id, delivery.seq, domain, exchange.exchange, ip, this.zone.name, zoneAddress.name, zoneAddress.address);
                        ...
                        let options = {
                            ...
                            logger: ('logger' in this.zone ? this.zone.logger : config.log.queue) ? {
                            ...
                            debug: ('logger' in this.zone ? this.zone.logger : config.log.queue)
                        };
                        ...
                        plugins.handler.runHooks('sender:connect', [delivery, options], err => {
                            ...
                            connection.once('error', err => {
                                ...
                                if (!connected) {
                                    // try next host
                                    if (mxTry >= exchanges.length) {
                                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s Failed to connect to %s[%s] for %s from %s (%s[%s])', delivery.id, delivery.seq, exchange.exchange, ip, domain, this.zone.name, zoneAddress.name, zoneAddress.address);
                                    }
                                    return tryConnectIP();
                                }

                                log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s Unexpected MX error', delivery.id, delivery.seq);
                                log.error('Sender/' + this.zone.name + '/' + process.pid, err.message);
                                return callback(err);
                            });
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s INITIALIZING (mx=%s mta=%s)', delivery.id, delivery.seq, options.host, options.localAddress);
                            connection.connect(() => {
                                log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s INITIALIZED (mx=%s mta=%s) (%s)', delivery.id, delivery.seq, options.host, options.localAddress, returned ? 'returned' : 'done');
                                if (returned) {
                                    return;
                                }
    ...
    deferDelivery(delivery, ttl, smtpResponse, callback) {
        this.sendCommand({
            cmd: 'DEFER',
            id: delivery.id,
            seq: delivery.seq,
