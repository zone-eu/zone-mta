    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < deferTimes.length) {
            ...
            return this.releaseDelivery(delivery, (err, released) => {
                ...
                if (released) {
                    // The bounce is part of this delivery's outcome. Wait until it has been
                    // queued before allowing a graceful shutdown to mark the send operation
                    // complete and exit the process.
                    return this.sendBounceMessage(delivery, bounce, smtpResponse, callback);
                } else {
