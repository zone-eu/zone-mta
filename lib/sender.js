...
class Sender extends EventEmitter {
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            send((err, info) => {
                                ...
                                log.info(
                                    this.logName,
                                    '%s.%s ACCEPTED from=%s[%s] to=%s mx=%s body=%s md5=%s[%s] (%s)',
                                    delivery.id,
                                    delivery.seq,
                                    delivery.from,
                                    (delivery.envelope && delivery.envelope.from) || delivery.from,
                                    (delivery.envelope && delivery.envelope.to) || delivery.recipient,
                                    connection.options.servername || delivery.domain,
                                    delivery.sentBodySize || -1,
                                    (delivery.sentBodyHash || '?').substr(0, 12),
                                    delivery.md5Match ? 'MD5_OK' : 'MD5_FAIL',
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < ttlMinutes.length) {
            ...
            log.info(
                this.logName,
                '%s.%s DEFERRED[%s] from=%s[%s] to=%s mx=%s (%s)',
                delivery.id,
                delivery.seq,
                bounce.category,
                delivery.from || '<>',
                (delivery.envelope && delivery.envelope.from) || delivery.from || '',
                (delivery.envelope && delivery.envelope.to) || delivery.recipient,
                (connection && connection.options.servername) || delivery.domain,
                smtpResponse
            );
            ...
            log.info(
                this.logName,
                '%s.%s REJECTED[%s] from=%s[%s] to=%s mx=%s (%s)',
                delivery.id,
                delivery.seq,
                bounce.category,
                delivery.from || '<>',
    ...
    signMessage(delivery) {
        ...
        // Get domain name using 'From' header
        let domainName = delivery.from.split('@').pop().replace(/>$/, '').trim().toLowerCase();

        // Load keys if exists for retrieved domain
        if (dkimKeys.has(domainName)) {
            delivery.dkim.keys = dkimKeys.get(domainName);
        }
    ...
    sendBounceMessage(delivery, bounce, smtpResponse) {
        if (/^mailer-daemon@/i.test(delivery.from) || !delivery.from) {
            log.info(
                ...
                delivery.seq,
                delivery.from || '<>',
                JSON.stringify(delivery.from || '').replace(/"/g, '').trim() || '<>'
            );
            return;
        }
        ...
        if (/^mailer-daemon@/i.test(delivery.parsedEnvelope.from)) {
            log.info(
                ...
                delivery.from || '<>',
                'From',
                JSON.stringify(delivery.parsedEnvelope.from || '<>').replace(/"/g, '').trim() || '<>'
            );
            return;
        }
