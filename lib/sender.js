                ...
                this.zone.speedometer(this.ref, () => { // check throttling speed
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            let fetchUrl = 'http://' + config.api.hostname + ':' + config.api.port + '/fetch/' + delivery.id + '?body=yes';
                            let messageFetch = fetch(fetchUrl);
                            ...
                            messageFetch.once('error', err => {
                                if (messageStream) {
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s FETCHFAIL [%s] url=%s fetch: %s', delivery.id, delivery.seq, connection.id, fetchUrl, JSON.stringify(String(err.stack)));
                                    messageStream.emit('error', err);
                                } else {
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s UNEXPECTED fetchfail [%s] url=%s fetch: %s', delivery.id, delivery.seq, connection.id, fetchUrl, JSON.stringify(String(err.stack)));
                                }
                            });
                            ...
                            connection.send({
                                ...
                                if (messageFetch.readable) {
                                    let readsize = 0;
                                    messageFetch.on('readable', () => {
                                        let chunk;
                                        while ((chunk = messageFetch.read()) !== null) {
                                            readsize += chunk.length;
                                        }
                                    });
                                    messageFetch.once('end', () => {
                                        log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s UNCLOSED [%s] Socket was not closed url=%s readsize=%s', delivery.id, delivery.seq, connection.id, fetchUrl, readsize);
                                    });
                                }
