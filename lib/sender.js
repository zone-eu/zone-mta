...
class Sender extends EventEmitter {
                ...
                this.getConnection(delivery, (err, connection) => {
                    ...
                    plugins.handler.runHooks('sender:headers', [delivery], err => {
                        ...
                        connection.send({
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s ACCEPTED to=%s mx=%s (%s)', delivery.id, delivery.seq, delivery.recipient, connection.options.name, this.formatSMTPResponse(info.response));
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if ((bounce = bounces.check(err.response)).action !== 'reject' || deferredCount > 6) {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s DEFERRED[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.recipient, connection && connection.options.name || delivery.domain, smtpResponse);
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s REJECTED[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.recipient, connection && connection.options.name || delivery.domain, smtpResponse);
