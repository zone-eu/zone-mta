    ...
    sendNext() {
        ...
        this.sendCommand('GET', (err, delivery) => {
            ...
            if (!delivery || !delivery.id) {
                this.emptyChecks++;
                setTimeout(() => continueSending(), Math.min(Math.pow(this.emptyChecks, 2), 1000) * 10).unref();
                return;
            }
            this.emptyChecks = 0;
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s FETCHED for delivery (from=%s to=%s)', delivery.id, delivery.seq, delivery.from, delivery.recipient);
            ...
            plugins.handler.runHooks('sender:fetch', [delivery], err => {
                ...
                this.zone.speedometer(this.ref, () => { // check throttling speed
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] CONNECTED mx=%s[%s]', delivery.id, delivery.seq, connection.id, connection.options.servername, connection.options.host);
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery], err => {
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] SENDING (from=%s to=%s size=%s)', delivery.id, delivery.seq, connection.id, delivery.envelope.from, delivery.envelope.to, messageSize);
                            ...
                            connection.send({
                                ...
                                if (!err && info && connectionDone) {
                                    ...
                                    log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] UNEXPECTED message delivery: %s', delivery.id, delivery.seq, connection.id, JSON.stringify(info));
                                }
                        ...
                        plugins.handler.runHooks('sender:connect', [delivery, options], err => {
                            ...
                            let _onError = err => {
                                ...
                                if ((err.code === 'ETLS' || /SSL23_GET_SERVER_HELLO|\/deps\/openssl/.test(err.message)) && !this.tlsDisabled.has(ip)) {
                                    ...
                                    log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] Failed to connect to %s[%s] using STARTTLS, proceeding with plaintext', delivery.id, delivery.seq, connId, exchange.exchange, ip);
                                ...
                                if (!connected) {
                                    // try next host
                                    if (mxTry >= exchanges.length) {
                                        log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] ERRCONNECT %s[%s] for %s from %s (%s[%s]). "%s"', delivery.id, delivery.seq, connId, exchange.exchange, ip, domain, this.zone.name, zoneAddress.name, zoneAddress.address, err.message);
                                    }
                                    return tryConnectIP(err);
                                }

                                log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] Unexpected MX error. %s', delivery.id, delivery.seq, connId, err.message);
                                return callback(err);
                            };

                            connection.once('error', err => {
                                log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] CERR %s', delivery.id, delivery.seq, connId, err.message);
                                _onError(err);
                            });

                            connection.once('end', () => {
                                log.error('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] CEND %s', delivery.id, delivery.seq, connId, err.message);
                                connection.connected = false;
                                if (returned) {
                                    return;
                                }
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] INITIALIZING (mx=%s mta=%s)', delivery.id, delivery.seq, connId, options.host, options.localAddress);
                            connection.connect(() => {
                                log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s [%s] INITIALIZED (mx=%s mta=%s)', delivery.id, delivery.seq, connId, options.host, options.localAddress);
                                if (returned) {
                                    return;
                                }
