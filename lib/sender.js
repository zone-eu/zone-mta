...
class Sender extends EventEmitter {
    ...
    sendNext() {
        ...
        this.sendCommand('GET', (err, delivery) => {
            ...
            log.verbose('Sender/' + this.zone.name + '/' + process.pid, '%s.%s FETCHED for delivery (from=%s to=%s)', delivery.id, delivery.seq, delivery.from, delivery.to);
                ...
                this.getConnection(delivery, (err, connection) => {
                    ...
                    plugins.handler.runHooks('sender:headers', [delivery], err => {
                        ...
                        connection.send({
                            ...
                            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s ACCEPTED from=%s[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, delivery.from, delivery.envelope && delivery.envelope.from || delivery.from, delivery.recipient, connection.options.servername || delivery.domain, this.formatSMTPResponse(info.response));
    ...
    handleResponseError(delivery, connection, err, callback) {
        let maxDeferred = 6;
        let bounce = bounces.check(err.response, err.bounce);
        ...
        if (bounce.action !== 'reject' && deferredCount < maxDeferred) {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s DEFERRED[%s] from=%s[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.from, delivery.envelope && delivery.envelope.from || delivery.from, delivery.recipient, connection && connection.options.servername || delivery.domain, smtpResponse);

            return this.deferDelivery(delivery, ttl, smtpResponse, (err, deferred) => {
                if (err) {
            ...
            log.info('Sender/' + this.zone.name + '/' + process.pid, '%s.%s REJECTED[%s] from=%s[%s] to=%s mx=%s (%s)', delivery.id, delivery.seq, bounce.category, delivery.from, delivery.envelope && delivery.envelope.from || delivery.from, delivery.recipient, connection && connection.options.servername || delivery.domain, smtpResponse);
    ...
    getConnection(delivery, connectionCallback) {
        ...
        let resolveMx = (domain, next) => {
            ...
            plugins.handler.runHooks('sender:mx', [delivery, exchanges], err => {
                ...
                iptools.resolveMx(domain, this.zone, next);
            });
        };
