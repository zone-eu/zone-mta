...
const getConnectionTlsLogData = connection => {
    let socket = connection?._socket;
    let cipher = typeof socket?.getCipher === 'function' ? socket.getCipher() : false;

    return {
        secure: !!connection?.secure,
        tls: !!connection?.secure,
        tlsVersion: typeof socket?.getProtocol === 'function' ? socket.getProtocol() : false,
        tlsCipher: cipher?.name,
        tlsAuthorized: typeof socket?.authorized === 'boolean' ? socket.authorized : false,
...
class Sender extends EventEmitter {
            ...
            cache: {
                set: async (domain, policy) => {
                    ...
                    try {
                        ...
                        emitGelf({
                            short_message: `${gelfCode('MTA_STS_REDIS_ERROR')} MTA-STS Redis error`,
                            _stack: err && err.stack ? err.stack : undefined,
                            _logger: this.logName + '/MTA-STS',
                            _domain: domain,
    ...
    _checkClosed() {
        if (!this.closing || this.closed || this.activeSendOperations) {
            return;
        }
        this.closed = true;
        this.emit('closed');
    ...
    sendNext() {
        ...
        this.sendCommand('GET', (err, delivery) => {
            if (err) {
                ...
                emitGelf({
                    short_message: `${gelfCode('QUEUE_REQUEST_FAILED')} Failed to fetch delivery from queue`,
                    _stack: err && err.stack ? err.stack : undefined,
                    _logger: this.logName,
                    _zone: this.zone && this.zone.name,
                    _failure_msg: err.message
                });
            ...
            if (!delivery || !delivery.id) {
                this.emptyChecks++;
                setTimeout(() => continueSending(), 1000);
                return;
            }
                    ...
                    this.getConnectionWithCache(delivery, (err, connection) => {
                        ...
                        if (!connection.http) {
                            ...
                            connection.once('error', err => {
                                ...
                                let remotePort =
                                    (connection && connection.options && connection.options.port) ||
                                    (connection && connection._socket && connection._socket.remotePort) ||
                                    delivery.mxPort ||
                                    this.zone.port ||
                                    25;
                                ...
                                log.error(
                                    this.logName,
                                    'id=%s %s.%s CONNECTION [%s] dst=%s:%s %s',
                                    delivery.sessionId,
                                    delivery.id,
                                    delivery.seq,
                                    connection.id,
                                ...
                                emitGelf({
                                    short_message: `${gelfCode('SMTP_CONNECTION_ERROR')} SMTP connection error`,
                                    _stack: err && err.stack ? err.stack : undefined,
                                    _logger: this.logName,
                        ...
                        plugins.handler.runHooks('sender:headers', [delivery, connection], err => {
                            ...
                            messageFetch.once('error', err => {
                                if (messageStream) {
                                    ...
                                    emitGelf({
                                        short_message: `${gelfCode('QUEUE_FETCH_FAILED')} Failed to fetch message body`,
                                        _stack: err && err.stack ? err.stack : undefined,
                                        _logger: this.logName,
                                        _session_id: delivery.sessionId,
                                        _delivery_id: delivery.id,
                                    ...
                                    emitGelf({
                                        short_message: `${gelfCode('QUEUE_FETCH_UNEXPECTED')} Unexpected message fetch failure`,
                                        _stack: err && err.stack ? err.stack : undefined,
                                        _logger: this.logName,
                                        _session_id: delivery.sessionId,
                                        _delivery_id: delivery.id,
                            ...
                            let send = next => {
                                ...
                                if (!connection.http) {
                                    ...
                                    messageStream.once('end', () => {
                                        ...
                                        postFormData(connection.targetUrl, formData, {
                                            'User-Agent': config.uploads.userAgent
                                        })
                                            .then(({ statusCode, body }) => {
                                                if (statusCode > 299 || statusCode < 200) {
                                                    let err = new Error(
                                                        statusCode + ' Upload failed. Received ' + (body ? body.length : 0) + ' B response'
                                                    );

                                                    err.protocol = 'http';
                                                    err.httpUrl = connection.targetUrl;
                                                    err.httpResponse = statusCode;
                                                    err.responseCode = statusCode || 550;
                                                    return next(err);
                                                }
                            ...
                            send((err, info) => {
                                ...
                                if (!err && info && connectionDone) {
                                    ...
                                    emitGelf({
                                        short_message: `${gelfCode('DELIVERY_UNEXPECTED_STATE')} Delivery completed after connection ended`,
                                        _logger: this.logName,
                                        _session_id: delivery.sessionId,
                                        _delivery_id: delivery.id,
                                        _delivery_seq: delivery.seq,
                                ...
                                log.info(
                                    this.logName,
                                    'id=%s %s.%s ACCEPTED from=%s to=%s src=%s mx=%s message-id=%s (%s)',
                                    delivery.sessionId,
                                    delivery.id,
                                    delivery.seq,
                                ...
                                plugins.handler.remotelog(delivery.id, delivery.seq, 'ACCEPTED', {
                                    ...
                                    ...getConnectionTlsLogData(connection),
                                ...
                                this.releaseDelivery(delivery, (err, released) => {
                                    if (err) {
                                        ...
                                        emitGelf({
                                            short_message: `${gelfCode('DELIVERY_ACK_FAILED')} Failed to acknowledge delivered message`,
                                    ...
                                    plugins.handler.runHooks('sender:delivered', [delivery, info], err => {
                                        if (err) {
                                            ...
                                            emitGelf({
                                                short_message: `${gelfCode('SENDER_DELIVERED_HOOK_FAILED')} sender:delivered hook failed`,
                                                _stack: err && err.stack ? err.stack : undefined,
                                                _logger: this.logName,
                                                _session_id: delivery.sessionId,
                                                _delivery_id: delivery.id,
    ...
    handleResponseError(delivery, connection, err, callback) {
        ...
        if (bounce.action !== 'reject' && deferredCount < deferTimes.length) {
            ...
            log.info(
                this.logName,
                'id=%s %s.%s DEFERRED[%s] from=%s to=%s src=%s mx=%s message-id=%s (%s)',
                delivery.sessionId,
                delivery.id,
                delivery.seq,
                bounce.category,
            ...
            return this.deferDelivery(delivery, ttl, smtpLog, smtpResponse, bounce, (err, deferred) => {
                if (err) {
                    ...
                    emitGelf({
                        short_message: `${gelfCode('DELIVERY_DEFER_FAILED')} Failed to defer delivery`,
                        full_message: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
            ...
            log.info(
                this.logName,
                'id=%s %s.%s REJECTED[%s] from=%s to=%s src=%s mx=%s message-id=%s (%s)',
                delivery.sessionId,
                delivery.id,
                delivery.seq,
                bounce.category,
            ...
            return this.releaseDelivery(delivery, (err, released) => {
                if (err) {
                    log.error(this.logName, 'id=%s %s.%s %s', delivery.sessionId, delivery.id, delivery.seq, err.message);
                    emitGelf({
                        short_message: `${gelfCode('DELIVERY_RELEASE_FAILED')} Failed to release delivery`,
                        full_message: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
                ...
                if (released) {
                    // Queueing the bounce can take a while, so it does not block fetching the
                    // next delivery. It is still counted as an active operation, a graceful
                    // shutdown waits until the bounce has been queued before exiting.
                    this.activeSendOperations++;
                    setImmediate(() =>
                        this.sendBounceMessage(delivery, bounce, smtpResponse, () => {
                            this.activeSendOperations--;
                            this._checkClosed();
                        })
                    );
    ...
    getConnection(delivery, done) {
        ...
        let tryConnect = () => {
            ...
            let mxOptions = {
                ...
                dnsOptions: ipTools.getMxConnectDnsOptions(delivery.dnsOptions),
                ignoreMXHosts,
                mxLastError
            };
            ...
            mxOptions.connectError = (err, opts, connOpts) => {
                ...
                emitGelf({
                    short_message: `${gelfCode('MX_CONNECT_ERROR')} MX connection error`,
                    _stack: err && err.stack ? err.stack : undefined,
                    _logger: this.logName,
                    _session_id: delivery.sessionId,
                    _delivery_id: delivery.id,
                    _delivery_seq: delivery.seq,
                    _domain: opts.domain,
                    _local_hostname: connOpts.localHostname,
                    _local_address: connOpts.localAddress,
                    _remote_host: connOpts.host,
                    _remote_port: connOpts.port,
                    _failure_msg: err.message
                });
            };
            ...
            mxConnect(mxOptions, (err, mx) => {
                if (err) {
                    ...
                    let remoteHost = delivery.domain;
                    let remotePort = delivery.mxPort || this.zone.port || 25;

                    log.error(
                        this.logName,
                        'id=%s %s.%s ERRCONNECT domain=%s remote=%s:%s error=%s',
                        delivery.sessionId,
                        delivery.id,
                        delivery.seq,
                        delivery.domain,
                    ...
                    emitGelf({
                        short_message: `${gelfCode('MX_CONNECT_FAILED')} Could not connect to MX`,
                        _stack: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
                        _delivery_seq: delivery.seq,
                        _domain: delivery.domain,
                        _remote_host: remoteHost,
                        _remote_port: remotePort,
                        _failure_msg: err.message
                    });
                ...
                let options = {
                    ...
                    transactionLog: true,
                    logger: nodemailerLogger.create(logger),
                    debug: 'debug' in this.zone ? this.zone.debug : config.log.queue
                };

                let connection = new SMTPConnection(options);
                connection.logtrail = logtrail;
                let remoteHost = options.servername || options.host;
                let remotePort = options.port;
                ...
                let _onError = err => {
                    ...
                    err.mxHost = err.mxHost || remoteHost;
                    err.mxPort = err.mxPort || remotePort;
                    if (returned) {
                        return;
                    }
                    returned = true;
                    ...
                    if (tlsRetry.isNetworkTimeout(err)) {
                    ...
                    log.error(
                        this.logName,
                        'id=%s %s.%s ERRCONNECT [%s] Unexpected MX error. dst=%s:%s src=%s error="%s"',
                        delivery.sessionId,
                        delivery.id,
                        delivery.seq,
                        connId,
                    ...
                    emitGelf({
                        short_message: `${gelfCode('MX_UNEXPECTED_ERROR')} Unexpected MX error`,
                        _stack: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
                        ...
                        _local_address: delivery.localAddress,
                        _remote_host: remoteHost,
                        _remote_port: remotePort,
                        _failure_msg: err.message
                    });
                ...
                connection.once('error', err => {
                    log.error(
                        this.logName,
                        'id=%s %s.%s SMTPERR [%s] dst=%s:%s src=%s error="%s"',
                        delivery.sessionId,
                        delivery.id,
                        delivery.seq,
                        connId,
                    ...
                    emitGelf({
                        short_message: `${gelfCode('SMTP_SOCKET_ERROR')} SMTP socket error`,
                        _stack: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
    ...
    sendBounceMessage(delivery, bounce, smtpResponse, callback) {
        ...
        this.sendCommand(
            ...
            err => {
                if (err) {
                    ...
                    log.error(this.logName, 'id=%s %s.%s %s', delivery.sessionId, delivery.id, delivery.seq, err.message);
                    emitGelf({
                        short_message: `${gelfCode('BOUNCE_SEND_FAILED')} Failed to enqueue bounce message`,
                        _stack: err && err.stack ? err.stack : undefined,
                        _logger: this.logName,
                        _session_id: delivery.sessionId,
                        _delivery_id: delivery.id,
