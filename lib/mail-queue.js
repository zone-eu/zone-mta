...
class MailQueue {
    ...
    shift(zone, options, callback) {
        ...
        let tryNext = () => {
            // This is where most CPU happens. If we have a lot of sender processes then all of them come here to
            // fetch the next delivery item. The first instance that manages to retrieve one will lock it and others
            // will start over until all have found an available delivery item.
            // Running iterators in parallel in itself is not an issue, so using shared iterator would not give much
            // (iterator can't run next() calls in parallel).
            //
            // This is mostly an issue if we have a lot of sender instances for the same Sending Zone.
            // For example if we have 4 processes with 10 connections for a zone, it means we have 40 instances.
            // Once all these instances start asking new messages from the same queue the first one gets the first available
            // delivery and locks it (1 iteration), the next one sees that the first one is locked and
            // gets the second one (+2 iterations), third one adds 3 iterations, etc. For 40 instances we might call
            // iterator.next for about 1000 times before all 40 get a delivery to handle, so it's O(n2)
            ...
            iterator.next((err, key, value) => {
                ...
                if (this.closing) {
                    // make sure we close the iterator
                    return callback(null, false);
                }
                if (!key) {
    ...
    getSharedIterator(name, options) {
        if (!this.iterators.has(name)) {
            ...
            let getNext = done => {
                if (!iterator) {
                    iterator = this.db.db.iterator(options);
                }
                iterator.next((err, key, value) => {
                    if (err) {
                        iterator = false;
            ...
            let sharedIterator = {
                ...
                next: callback => {
                    ...
                    let iterate = () => {
                        ...
                        let done = queue.shift();
                        if (closing) {
                            return setImmediate(done);
                        }
                        getNext((err, key, value) => {
                            setImmediate(() => {
                                done(err, key, value);
                            });
                            iterate();
            ...
            this.iterators.set(name, sharedIterator);
        }
