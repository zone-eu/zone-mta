...
class MailQueue {
    ...
    deferDelivery(delivery, ttl, response, callback) {
        ...
        this.db.batch(ops, err => {
            ...
            log.info('Queue', '%s.%s UNLOCK (key="%s")', delivery.id, delivery.seq, delivery._lock);
            this.locks.release(delivery._lock);
    ...
    decStats(zone, domain) {
        ...
        if (this.stats.zoneQueued.has(zone)) {
            ...
            if (this.stats.zoneQueued.get(zone).domains.has(domain) &&
                --this.stats.zoneQueued.get(zone).domains.get(domain).queued <= 0) {
