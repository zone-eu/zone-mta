...
class MailQueue {
    ...
    shift(zone, options, callback) {
        ...
        let skipDomains = this.locks.listSkipDomains();
        if (skipDomains) {
            query.domain = {
                $nin: skipDomains
            };
        ...
        let tryNext = () => {
            ...
            collection.findOne(query, (err, delivery) => {
                if (err) {
                    return callback(err);
                }
            ...
            this.get(options, (err, result) => {
                ...
                let checkMessage = () => {
                    ...
                    let message = {
                        ...
                        status: 'QUEUED',
                        lock: this.locks.locks.get('lock ' + id + ' ' + valparts[3])
                    };
        ...
        collection.updateMany({
            ...
            collection.findOne({}, {
                ...
                if (!delivery) {
                    ...
                    clearUntil = delivery.queued.getTime();
                }
