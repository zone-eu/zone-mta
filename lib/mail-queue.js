...
class MailQueue {
    ...
    push(id, envelope, callback) {
        ...
        let processRecipients = () => {
            ...
            if (!deliveryZone && senderDomain) {
                ...
                log.info('Queue', 'Detected Zone %s for %s:%s by sender %s', deliveryZone, id, recipient, senderDomain);
            }
            ...
            if (!deliveryZone && recipientDomain) {
                ...
                log.info('Queue', 'Detected Zone %s for %s:%s by recipient %s', deliveryZone, id, recipient, recipientDomain);
            }
    ...
    shift(zone, options, callback) {
        ...
        if (!domain) {
            // index by zone
            index = this.lastZoneKey ? this.lastZoneKey : 'delivery ' + encodeURIComponent(zone) + ' ';
        ...
        let iterator = this.db.db.iterator({
            gt: index,
            lt: end,
            keys: true,
            values: true,
        ...
        let reqId = Math.round(Math.random() * 0x1000).toString(16);
        let reqIt = 0;
        let reqIndex = index;
        console.log('%s create (%s)', reqId, index);

        // iterate over all keys to find first matching and unlocked delivery
        let tryNext = () => {
            if (this.closing) {
                // make sure we close the iterator
                return iterator.end(() => callback(null, false));
            }
            console.log('%s next %s', reqId, reqIt++);
            iterator.next((err, key, value) => {
                ...
                if (typeof key === 'undefined') {
                    // reached end, close the iterator
                    console.log('%s ending', reqId);
                    return iterator.end(err => {
                        if (err) {
                            console.log('%s ending error', reqId);
                            return callback(err);
                        }
                        if (domain && !options.toDomainOnly) {
                            ...
                            // start over
                            console.log('%s ending startover', reqId);
                ...
                if ((!domain && this.isSkipDomain(zone, deliveryDomain)) || !this.acquireLock(lockKey, lockOwner)) {
                    ...
                    console.log('%s lock (%s: %s)', reqId, domain || zone, lockKey);
                    if (domain) {
                        console.log(this.skipDomains);
                    }

                    return setImmediate(tryNext);
                }
                console.log('%s match (%s)', reqId, lockKey);
                if (!domain) {
                ...
                this.db.get(refKey, {
                    ...
                    if (err) {
                        if (err.name === 'NotFoundError') {
                            ...
                            console.log('%s ending release not found', reqId);
                            return this.releaseDelivery({
                                ...
                                if (err) {
                                    console.log('%s ending release error', reqId);
                                    return iterator.end(() => callback(err));
                                }
                                setImmediate(tryNext);
                            });
                        }
                        console.log('%s ending release', reqId);
                        return iterator.end(() => callback(err));
                    }
                    return iterator.end(err => {
                        if (err) {
                            this.releaseLock(lockKey);
                            console.log('%s ending itend', reqId);
                            return callback(err);
                        }
                        // Store used lock key in the delivery object
                        delivery._lock = lockKey;
                        console.log('%s return match', reqId);
