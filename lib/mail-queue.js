...
class MailQueue {
        ...
        let recipientsProcessed = () => {
            ...
            this.db.batch(ops, err => {
                ...
                inserted.forEach(entry => {
                    this.incStats(entry.zone, entry.domain);
                    if (entry.deferred) {
                        this.incDeferred(entry.zone, entry.domain);
                    }
                });
    ...
    shift(zone, options, callback) {
        ...
        let tryNext = () => {
            iterator.next((err, key) => {
                ...
                if (!this.locks.lock(lockKey, zone, deliveryDomain, lockOwner, maxConnections)) {
     ...
     * @param {Number} response SMTP response or description
     * @param {Function} callback Run once the message is removed from active queue
     */
    deferDelivery(delivery, ttl, response, callback) {
        ...
        delivery._deferred.response = response;
    ...
    recoverDeferred(callback) {
        ...
        stream.on('end', () => {
            ...
            recovered.forEach(entry => {
                this.decDeferred(entry.zone, entry.domain);
            });
        ...
        this.get(options, (err, result) => {
            ...
            let checkMessage = () => {
                ...
                let checkDeferred = done => {
                    this.db.get('deferred:key delivery ' + encodeURIComponent(message.zone) + ' ' + id + ' ' + encodeURIComponent(message.recipient), (err, deferred) => {
                        if (err) {
                            if (err.name === 'NotFoundError') {
                                // not deferred
                                return done();
    ...
    incDeferred(zone, domain) {
        ...
        if (this.stats.zoneQueued.has(zone)) {
            let zoneQueued = this.stats.zoneQueued.get(zone);
            zoneQueued.deferred++;
            if (zoneQueued.domains.has(domain)) {
                zoneQueued.domains.get(domain).deferred++;
            }
        }
    }
    ...
    decDeferred(zone, domain) {
        ...
        if (this.stats.zoneQueued.has(zone)) {
            let zoneQueued = this.stats.zoneQueued.get(zone);
            zoneQueued.deferred--;
            if (zoneQueued.domains.has(domain) && zoneQueued.domains.get(domain).deferred) {
    ...
    incStats(zone, domain) {
        ...
        if (!this.stats.zoneQueued.has(zone)) {
            ...
            this.stats.zoneQueued.set(zone, zoneQueued);
        } else {
            zoneQueued = this.stats.zoneQueued.get(zone);
            zoneQueued.queued++;
        }

        // Increment zone+domain counter
        if (!zoneQueued.domains.has(domain)) {
            zoneQueued.domains.set(domain, {
                queued: 1,
                deferred: 0
            });
        } else {
            zoneQueued.domains.get(domain).queued++;
        }
    ...
    decStats(zone, domain) {
        ...
        if (this.stats.domainQueued.has(domain) && --this.stats.domainQueued.get(domain).queued <= 0) {
        ...
        if (this.stats.zoneQueued.has(zone)) {
            let zoneQueued = this.stats.zoneQueued.get(zone);
            zoneQueued.processed++;
            zoneQueued.queued--;
            if (zoneQueued.domains.has(domain) &&
                --zoneQueued.domains.get(domain).queued <= 0) {
                zoneQueued.domains.delete(domain);
            }
        }
    }
