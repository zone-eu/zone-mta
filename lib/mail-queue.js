...
class MailQueue {
                ...
                this.db.get(refKey, {
                    ...
                    if (err) {
                        if (err.name === 'NotFoundError') {
                            ...
                            return this.db.get(zoneKey, (err, value) => {
                                log.info('Queue', 'UNKNOWN KEY <%s>', key);
                                if (err) {
                                    log.info('Queue', 'ZONEKEY <%s> error', zoneKey, err);
                                } else if (value) {
                                    log.info('Queue', 'ZONEKEY <%s> exists (%s)', zoneKey, value);
                                }
                                return this.db.get(domainKey, (err, value) => {
                                    log.info('Queue', 'UNKNOWN KEY <%s>', key);
                                    if (err) {
                                        log.info('Queue', 'DOMAINKEY <%s> error', domainKey, err);
                                    } else if (value) {
                                        log.info('Queue', 'DOMAINKEY <%s> exists (%s)', domainKey, value);
                                    }
    ...
    releaseDelivery(delivery, callback) {
        log.info('Queue', 'RELEASING <%s>', delivery._zoneKey);
    ...
    deferDelivery(delivery, ttl, callback) {
        log.info('Queue', 'DEFERRING <%s>', delivery._zoneKey);
    ...
    recoverDeferred(callback) {
        ...
        stream.on('readable', () => {
            ...
            while ((data = stream.read()) !== null) {
                ...
                log.info('Queue', 'RECOVERING <%s>', zoneKey);
