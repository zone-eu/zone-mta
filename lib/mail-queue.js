...
const queueSizeGauge = new promClient.Gauge('zonemta_queue_size', 'Current size of the queue', ['type']);
const blacklistedGauge = new promClient.Gauge('zonemta_blacklisted', 'Blacklisted addresses');
...
class MailQueue {
    ...
    shift(zone, options, callback) {
        ...
        let tryNext = () => {
            collection.findOneAndUpdate(query, {
                ...
                let lockKey = 'lock ' + delivery.id + ' ' + delivery.recipient;
                    ...
                    this.mongodb.collection('suppressionlist').findOne({
                        ...
                        if (suppressed) {
                            return this.releaseDelivery(delivery, err => {
                                ...
                                log.info('Queue', '%s.%s DROP[suppressed] Recipient %s was found from suppression list', delivery.id, delivery.seq, delivery.recipient);
        ...
        collection.find(query, {
            ...
            let releaseNext = () => {
                cursor.nextObject((err, delivery) => {
                    ...
                    delivery._lock = 'lock ' + delivery.id + ' ' + delivery.recipient;
    ...
    getInfo(id, callback) {
        ...
        this.mongodb.collection(this.options.collection).find({
            id
        }).sort({
            seq: 1
            ...
            this.getMeta(id, (err, meta) => {
                ...
                let messages = deliveries.map(delivery => {
                    ...
                    let message = {
                        ...
                        seq: delivery.seq,
                        zone: delivery.sendingZone,
                        recipient: delivery.sendingZone,
                        status: deferred ? 'DEFERRED' : 'QUEUED',
                        lock: this.locks.locks.get('lock ' + delivery.id + ' ' + delivery.recipient)
                    };
                    if (deferred) {
    ...
    listQueued(zone, type, maxItems, callback) {
        ...
        this.mongodb.collection(this.options.collection).find(query).project({
            id: 1,
            sendingZone: 1,
            recipient: 1
        }).sort({
            _id: 1
        }).limit(maxItems).toArray((err, entries) => {
            if (err) {
                return callback(err);
            }
            ...
            return callback(null, entries.map(entry => ({
                id: entry.id,
    ...
    count(zones, type, callback) {
        ...
        let checkZone = (zone, type) => new Promise((resolve, reject) => {
            let query = {
                sendingZone: zone,
                queued: {
                    [type === 'deferred' ? '$gt' : '$lte']: new Date()
        ...
        Promise.all(zones.map(zone => checkZone(zone, type))).then(entries => {
