...
class MailQueue {
    ...
    push(id, envelope, callback) {
        ...
        let recipientsProcessed = () => {
            ...
            redisClient.multi().
            hincrby('scount', 'push', 1).
            hincrby('scount', 'push:ops', ops.length).
            exec(() => false);
    ...
    shift(zone, options, callback) {
        ...
        let tryNext = () => {
            iterator.next((err, key) => {
                ...
                // time we use it
                redisClient.hincrby('scount', 'next:key', 1, () => false);
                this.db.get(key, (err, value) => {
                    ...
                    redisClient.hincrby('scount', 'next:value', 1, () => false);
                    this.db.get(refKey, {
                        valueEncoding: 'json'
                    }, (err, delivery) => {
                        if (err) {
                            if (err.name === 'NotFoundError') {
                                ...
                                redisClient.hincrby('scount', 'next:404', 1, () => false);
    ...
    remove(id, seq, callback) {
        ...
        let deliveries = [];
        redisClient.hincrby('scount', 'remove:stream', 1, () => false);
        ...
        stream.on('end', () => {
            ...
            let releaseNext = () => {
                ...
                redisClient.hincrby('scount', 'remove:key', 1, () => false);
    ...
    getDelivery(id, seq, callback) {
        redisClient.hincrby('scount', 'getdelivery:key', 1, () => false);
        this.db.get('seq ' + id + ' ' + seq, (err, refKey) => {
            ...
            redisClient.hincrby('scount', 'getdelivery:value', 1, () => false);
            this.db.get(refKey, {
                valueEncoding: 'json'
            }, (err, delivery) => {
                if (err) {
    ...
    get(query, callback) {
        ...
        redisClient.hincrby('scount', 'get:stream', 1, () => false);
        let stream = this.db.createReadStream(query);
    ...
    deleteRange(query, callback) {
        ...
        redisClient.hincrby('scount', 'delete:stream', 1, () => false);
        ...
        let deleteKey = (key, next) => {
            redisClient.hincrby('scount', 'delete:key', 1, () => false);
            this.db.del(key, err => {
                if (returned) {
                    return drainKeys();
                }
    ...
    releaseDelivery(delivery, callback) {
        ...
        redisClient.multi().
        hincrby('scount', 'release', 1).
        hincrby('scount', 'release:ops', ops.length).
        exec(() => false);
        this.db.batch(ops, {
            sync: false
        }, err => {
    ...
    deferDelivery(delivery, ttl, responseData, callback) {
        ...
        redisClient.multi().
        hincrby('scount', 'defer', 1).
        hincrby('scount', 'defer:ops', ops.length).
        exec(() => false);
        this.db.batch(ops, err => {
            if (err) {
                return callback(err);
    ...
    recoverDeferred(itemKey, callback) {
        ...
        redisClient.hincrby('scount', 'recover:stream', 1, () => false);
        let stream = this.db.createReadStream(query);
        ...
        stream.on('readable', () => {
            ...
            redisClient.multi().
            hincrby('scount', 'recover:entry', 1).
            hincrby('scount', 'recover:ops', ops.length).
            exec(() => false);
            this.db.batch(ops);
        });
            ...
            this.get(options, (err, result) => {
                ...
                let checkMessage = () => {
                    ...
                    let checkDeferred = done => {
                        redisClient.hincrby('scount', 'checkdefer:key', 1, () => false);
                        this.db.get('deferred:key delivery-zone ' + encodeURIComponent(message.zone) + ' ' + id + ' ' + encodeURIComponent(message.recipient), (err, deferred) => {
                            ...
                            redisClient.hincrby('scount', 'checkdefer:value', 1, () => false);
    ...
    clearGarbage(callback) {
        ...
        this.getFirst({
            ...
            let deleted = 0;
            let collection = this.mongodb.collection(this.options.gfs + '.files');
            collection.find({
                uploadDate: {
    ...
    listQueued(type, zone, maxItems, callback) {
        ...
        redisClient.hincrby('scount', 'qlist:stream', 1, () => false);
    ...
    count(type, key, counter, callback) {
        ...
        redisClient.hincrby('scount', 'count:stream', 1, () => false);
        let stream = this.db.createReadStream({
            gt,
            lt,
            keys: true,
    ...
    init(callback) {
        ...
        if (!this.db) {
            mkdirp(this.options.db, err => {
                ...
                redisClient.hincrby('scount', 'create', 1, () => false);
                this.db = levelup(this.options.db, opts);

                this.db.on('ready', () => {
                    redisClient.hincrby('scount', 'ready', 1, () => false);
                    if (returned) {
                    ...
                    MongoClient.connect(this.options.mongodb, (err, database) => {
                        ...
                        this.mongodb.ensureIndex(this.options.gfs + '.files', {
                            uploadDate: -1
                        }, () => {
                            returned = true;
                ...
                this.db.once('error', err => {
                    redisClient.hincrby('scount', 'error', 1, () => false);
                    if (returned) {
