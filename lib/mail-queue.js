...
class MailQueue {
        ...
        this.db.batch(ops, {
            ...
            this.getFirst({
                ...
                if (!err && !item) {
                    ...
                    if (this.options.delayDelete > 0 && !delivery.skipDelayDelete) {
                        let ttl = Math.min(this.options.delayDelete, 7 * 24 * 3600 * 1000);
                        let expires = (Date.now() + ttl);
                        let deleteKey = 'deleted:item ' + expires + ' ' + this.seqIndex.short();
                        log.verbose('Queue', 'Delay deleting unreferenced message %s for %s ms', delivery.id, ttl);
                        let ops = [{
                            type: 'put',
                            key: deleteKey,
                            value: delivery.id
                            ...
                            type: 'put',
                            key: 'deleted:key ' + delivery.id,
                            value: expires
                        }];
                        this.db.batch(ops, err => callback(null, !err));
                    } else {
    ...
    getInfo(id, callback) {
        ...
        let loadMessage = (options, deleted) => {
            ...
            this.get(options, (err, result) => {
                ...
                if (deleted) {
                    this.getMeta(id, (err, meta) => {
                        ...
                        meta.isDeleted = true;
                        meta.expiresAfter = Number(result && result[0] && result[0].value) || 0;
                        callback(null, {
                            meta,
                            messages
                        });
    ...
    clearGarbage(callback) {
        ...
        let deleteRemains = () => {
            ...
            // find the oldest undelivered message
            this.getFirst({
                gt: 'ref ',
                lt: 'ref ~',
                keys: true,
                ...
                if (!refKey) {
                    ...
                    keyParts.shift();
                    clearUntil = Math.min(Math.round(parseInt(keyParts.shift().substr(0, 14), 16) / 0x1000), Date.now());
                }

                // find oldest message queued to be deleted
                this.getFirst({
                    gt: 'deleted:item ',
                    lt: 'deleted:item ' + Date.now() + ' ~',
                    keys: true,
                    values: false
                }, (err, refKey) => {
                    if (err) {
                    ...
                    if (refKey) {
                        let keyParts = refKey.split(' ');
                        let keyTTL = Number(keyParts[1]) || 0;
                        if (keyTTL && keyTTL < clearUntil) {
                            clearUntil = keyTTL;
                        }
                    }
                    ...
                    log.verbose('Queue', 'Deleting messages older than %s', new Date(clearUntil).toISOString().substr(0, 19).replace(/T/, ' ') + ' UTC');
        ...
        let deleteExpired = () => {
            this.getFirst({
                ...
                let ops = [{
                    ...
                    type: 'del',
                    key: 'deleted:key ' + delInfo.value
                }];
                this.db.batch(ops, err => {
                    if (err) {
                        return callback(err);
                    }
                    log.verbose('Queue', 'Deleting unreferenced message %s', delInfo.value);
                    this.streamer.delete('message ' + delInfo.value, err => {
                        if (err) {
                            return callback(err);
                        }
                        setImmediate(deleteExpired);
                    });
                });
            });
        };
