            ...
            mkdirp(this.options.db, err => {
                ...
                switch (this.options.backend) {
                    case 'redisdown':
                        {
                            let opts = {};
                            Object.keys(this.options.redisdown || {}).forEach(key => {
                                opts[key] = this.options.redisdown[key];
                            });
                            opts.db = redisdown;
                            this.db = levelup(this.options.db, opts);
                            this.streamer = redisStreamAccess(this.options.redisdown);
                            break;
                        }
                    case 'leveldown':
                    default:
                        this.db = levelup(this.options.db, this.options.leveldown);
                }
