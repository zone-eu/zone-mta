...
class MailQueue {
    ...
    constructor(options) {
        ...
        internalCounters.count('level:db');
    ...
    push(id, envelope, callback) {
        ...
        let recipientsProcessed = () => {
            ...
            internalCounters.count('level:push');
            internalCounters.count('level:push:ops', ops.length);
    ...
    shift(zone, options, callback) {
        ...
        let tryNext = () => {
            iterator.next((err, key) => {
                ...
                // time we use it
                internalCounters.count('level:shift:key');
                this.db.get(key, (err, value) => {
                    ...
                    internalCounters.count('level:shift:value');
                    this.db.get(refKey, {
                        valueEncoding: 'json'
                    }, (err, delivery) => {
                        if (err) {
                            if (err.name === 'NotFoundError') {
                                ...
                                internalCounters.count('level:shift:404');
    ...
    remove(id, seq, callback) {
        ...
        let deliveries = [];
        internalCounters.count('level:remove:stream');
        ...
        stream.on('end', () => {
            ...
            let releaseNext = () => {
                ...
                internalCounters.count('level:remove:key');
    ...
    getDelivery(id, seq, callback) {
        internalCounters.count('level:getdelivery:key');
        this.db.get('seq ' + id + ' ' + seq, (err, refKey) => {
            ...
            internalCounters.count('level:getdelivery:value');
            this.db.get(refKey, {
                valueEncoding: 'json'
            }, (err, delivery) => {
                if (err) {
    ...
    get(query, callback) {
        ...
        internalCounters.count('level:get:stream');
        let stream = this.db.createReadStream(query);
    ...
    deleteRange(query, callback) {
        ...
        internalCounters.count('level:delete:stream');
        ...
        let deleteKey = (key, next) => {
            internalCounters.count('level:delete:key');
            this.db.del(key, err => {
                if (returned) {
                    return drainKeys();
                }
    ...
    releaseDelivery(delivery, callback) {
        ...
        internalCounters.count('level:release');
        internalCounters.count('level:release:ops', ops.length);
        this.db.batch(ops, {
            sync: false
        }, err => {
    ...
    deferDelivery(delivery, ttl, responseData, callback) {
        ...
        internalCounters.count('level:defer');
        internalCounters.count('level:defer:ops', ops.length);
        this.db.batch(ops, err => {
            if (err) {
                return callback(err);
            }
    ...
    recoverDeferred(itemKey, callback) {
        ...
        internalCounters.count('level:recover:stream');
        let stream = this.db.createReadStream(query);
        ...
        stream.on('readable', () => {
            ...
            internalCounters.count('level:recover');
            internalCounters.count('level:defer:ops', ops.length);
            this.db.batch(ops);
        });
            ...
            this.get(options, (err, result) => {
                ...
                let checkMessage = () => {
                    ...
                    let checkDeferred = done => {
                        internalCounters.count('level:getinfo:deferred');
                        this.db.get('deferred:key delivery-zone ' + encodeURIComponent(message.zone) + ' ' + id + ' ' + encodeURIComponent(message.recipient), (err, deferred) => {
                            ...
                            internalCounters.count('level:getinfo:value');
    ...
    listQueued(type, zone, maxItems, callback) {
        ...
        internalCounters.count('level:list:stream');
    ...
    count(type, key, counter, callback) {
        ...
        internalCounters.count('level:count:stream');
        let stream = this.db.createReadStream({
            gt,
            lt,
            keys: true,
    ...
    init(callback) {
        ...
        if (!this.db) {
            mkdirp(this.options.db, err => {
                ...
                internalCounters.count('level:init');
                this.db = levelup(this.options.db, opts);

                this.db.on('ready', () => {
                    internalCounters.count('level:ready');
                    if (returned) {
                ...
                this.db.once('error', err => {
                    internalCounters.count('level:error');
                    if (returned) {
