...
class APIServer {
    ...
    setupRoutes() {
        ...
        this.server.del('/message/:id/:seq', (req, res, next) => {
            if (config.api.user || config.api.pass) {
                if (!req.username || req.username !== config.api.user || !req.authorization.basic || req.authorization.basic.password !== config.api.pass) {
                    res.json(403, {
                        error: 'Authentication required'
                    });
                    return next();
