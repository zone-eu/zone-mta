...
class APIServer {
    ...
    setupRoutes() {
        ...
        // Swagger UI
        this.server.get("/swagger/:file", (req, res, next) => {
            const filePath = path.join(swaggerUiDist.getAbsoluteFSPath(), req.params.file);
        ...
        this.server.get("/swagger", (req, res, next) => {
            res.redirect("/swagger/index.html", next);
        });

        // Serve the Swagger spec JSON at the correct path: /swagger.json
        this.server.get("/swagger.json", (req, res, next) => {
            // Get Info from config
            const infoTitle = config.api.swaggerTitle || 'ePost Send Secure Zone-MTA API';
            const infoDescription = config.api.swaggerDescription || 'This REST API can be use to send secure email messages over Zone-MTA.';
            const infoVersion = config.api.swaggerVersion || '1.0.0';
            const swaggerDocs = {
                ...swaggerSpec,
                servers: [{ url: "/" }], // Dynamically set the server URL
                info: { title: infoTitle, description: infoDescription, version: infoVersion } // Dynamically set the swagger info
            };
            res.send(swaggerDocs); // Send the Swagger spec as JSON
            next();
        });
    }
}
