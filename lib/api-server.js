...
class APIServer {
    ...
    setupRoutes() {
        ...
        this.server.get('/counter/:type/:zone', (req, res, next) => {
            ...
            let zone = req.params.zone ? req.params.zone : Object.keys(config.zones).filter(zone => config.zones[zone] && !config.zones[zone].disabled).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
        ...
        this.server.put('/message/:id/:seq', (req, res, next) => {
            ...
            this.queue.update(req.params.id, req.params.seq, {
                $set: {
                    queued: new Date()
                }
            }, err => {
        ...
        this.server.get('/suppressionlist', (req, res, next) => {
            ...
            this.queue.mongodb.collection('suppressionlist').find().sort({
                created: -1
