    ...
    setupRoutes() {
        ...
        this.server.post('/send-raw', (req, res, next) => {
            ...
            plugins.handler.runHooks('api:mail', [envelope, session], err => {
                ...
                this.maildrop.add(envelope, req, (err, response) => {
                    if (err) {
                        if (err.name === 'SMTPResponse') {
                            return res.json(200, {
                                message: err.message
        ...
        this.server.get('/zones', (req, res, next) => {
            let zones = Object.keys(config.zones).map(zone => ({
                name: zone
        ...
        this.server.get('/queued/:type/:zone', (req, res, next) => {
            ...
            this.queue.listQueued(req.params.type, req.params.zone, 1000, (err, list) => {
                if (err) {
                    res.json(500, {
                        error: err.message
                    });
                ...
                res.json({
                    ...
                    type: req.params.type === 'deferred' ? 'deferred' : 'queued',
                    list
                });
