...
// An api:mail hook that refuses a message on purpose carries its own status, the same way an SMTP
// reject carries responseCode - a plugin that sheds load needs to answer 429 or 503, and a plugin
// that validates needs a 4xx. Without a status the error is ours rather than a decision, so it
// stays a generic 500 and the message is not passed on to the caller.
function sendHookError(res, err) {
    let code = Number(err.responseCode);
...
class APIServer {
    ...
    setupRoutes() {
        ...
        this.server.put('/store/:id', (req, res, next) => {
            ...
            this.queue.store(id, req, (err, storedId, filesId) => {
                if (err) {
                    res.json(500, {
                        error: err.message
                    });
                ...
                // Nothing queues what this endpoint stores, so the marker ends with the upload.
                // A marker left behind has the GC remove the body, so say so rather than 200.
                this.queue.clearPending(filesId, pendingErr => {
                    if (pendingErr) {
                        log.error('API', '%s stored but its marker could not be cleared, so it will be collected. %s', id, pendingErr.message);
                        res.json(500, {
