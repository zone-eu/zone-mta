...
class APIServer {
    ...
    setupRoutes() {
        ...
        this.server.post('/send', (req, res, next) => {
            ...
            envelope.origin = req.connection.ddress;
            ...
            let addMessage = () => {
                userTools.getSenderConfig(envelope.from, session, (err, senderConfig) => {
                    if (err) {
                        res.json(500, {
                            error: err.message
                        });
            ...
            if (config.feeder.authentication) {
                ...
                plugins.handler.runHooks('feeder:auth', [{
