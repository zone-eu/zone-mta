...
class QueueLocker {
    ...
    listSkipDomains(zone) {
        if (this.zones.has(zone)) {
            let zoneData = this.zones.get(zone);
            if (!zoneData.skipCached) {
                zoneData.skipCached = Array.from(zoneData.skip);
            }
            return zoneData.skipCached.length ? zoneData.skipCached : false;
        }
        return false;
    }
}
