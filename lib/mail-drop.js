        ...
        plugins.handler.runHooks('message:store', [envelope, dkimStream], err => {
            ...
            this.queue.store(id, dkimStream, err => {
                if (err) {
                    ...
                    log.error('Feeder', '%s; Error processing incoming message %s: %s (From: %s; To: %s)', envelope.transaction, envelope.messageId || id, err.message, envelope.from, toList);
                    return callback(err);
                }
                ...
                plugins.handler.runHooks('message:queue', [envelope], err => {
                    ...
                    this.queue.setMeta(id, envelope, err => {
                        ...
                        this.queue.push(id, envelope, err => {
                            ...
                            log.info('Feeder', '%s; RECEIVED %s %s (From: %s; To: %s)', envelope.transaction, id, envelope.messageId, envelope.from, toList);
