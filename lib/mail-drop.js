    ...
    add(envelope, source, callback) {
        ...
        plugins.handler.runHooks('message:store', [envelope, dkimStream], async (err) => {
            ...
            if(!envelope.signEML) {
                ...
                let signedStream = new BodyHashStream('relaxed/relaxed', envelope.dkim.hashAlgo);
                signedStream.on('hash', bodyHash => {
                    // store relaxed body hash for signing
                    envelope.dkim.bodyHash = bodyHash;
                    envelope.bodySize = signedStream.byteLength;
                    // re-write values for messageInfo after signing/encryptiong
                    // BODY SIZE
                    messageInfo.body = envelope.bodySize || 0;
                    // SUBJECT
                    let subjectUpdated = envelope.headers.getFirst('subject');
                    try {
                    ...
                    messageInfo.subject = subjectUpdated;
                    // RECIPIENTS/TO
                    messageInfo.to = [].concat(envelope.to || []).join(',') || '<>';
                });

                encryptedStream.pipe(signedStream);

                signedStream.pipe(messageHashStream);
                signedStream.once('error', err => {
                    messageHashStream.emit('error', err);
                });
            }
