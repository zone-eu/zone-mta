...
class MailDrop {
    ...
    add(envelope, source, callback) {
        ...
        plugins.handler.runHooks('message:store', [envelope, dkimStream], err => {
            ...
            // store stream to db
            this.queue.store(id, messageHashStream, (err, storedId, filesId) => {
                // Removing is the master's job from here on, so this process stops beating for a
                // marker it has handed over. Left running, the beat stops only when it next finds
                // the marker gone - and if the master's own clear fails, not even then.
                const handOver = done => {
                    if (filesId) {
                        gridfsPending.stopPending(filesId);
                    }
                    return this.queue.removeMessage(id, done);
                };
                ...
                plugins.handler.runHooks('message:queue', [envelope, messageInfo], err => {
                    if (err) {
                        return setImmediate(() => handOver(() => callback(err)));
                    }
                    ...
                    this.queue.setMeta(id, envelope, err => {
                        ...
                        this.queue.push(id, envelope, err => {
                            // The message belongs to a delivery now, so the marker that covered
                            // the gap between storing it and queueing it can go. A failure here
                            // leaves it for the GC, which is the whole point of it existing.
                            if (!err && filesId) {
                                this.queue.clearPending(filesId, pendingErr => {
                                    if (pendingErr) {
                                        log.info('Queue/' + process.pid, 'id=%s %s failed to clear the upload marker. %s', envelope.sessionId, id, pendingErr.message);
                                    }
                                });
                            }
