...
class MailDrop {
    ...
    add(id, envelope, stream, callback) {
        ...
        let rspamdStream;
        let dkimStream;
        let dkim = envelope.dkim || {};
        if ('dkim' in envelope) {
            delete envelope.dkim;
        }
        dkim.hashAlgo = dkim.hashAlgo || (this.options.dkim && this.options.dkim.hashAlgo) || 'sha256';
        dkim.debug = this.options.dkim && this.options.dkim.debug;
        ...
        dkimStream = new DkimRelaxedBody(dkim);
        dkimStream.on('hash', bodyHash => {
            // store relaxed body hash for signing
            dkim.bodyHash = bodyHash;
        });

        plugins.runAnalyzerHooks(envelope, stream, rspamdStream);
        ...
        this.queue.store(id, dkimStream, err => {
            ...
            plugins.runHooks('message:store', [envelope, message.headers], err => {
                ...
                this.queue.setMeta(id, {
                    dkim,
                    bodySize: dkimStream.byteLength,
                    headers: message.headers ? message.headers.getList() : [],
                    spam,
