        ...
        plugins.handler.runHooks('message:store', [envelope, dkimStream], err => {
            if (err) {
                if (err.name === 'SMTPResponse') {
                    return setImmediate(() => callback(null, err.message));
                }
            ...
            this.queue.store(id, dkimStream, err => {
                if (err) {
                    ...
                    log.error('Feeder', '%s NOQUEUE "%s" (message-id=%s from=%s to=%s)', id, err.message, envelope.messageId,  envelope.from, toList);
                    return callback(err);
                }
                ...
                plugins.handler.runHooks('message:queue', [envelope], err => {
                    ...
                    this.queue.setMeta(id, envelope, err => {
                        ...
                        this.queue.push(id, envelope, err => {
                            ...
                            log.info('Feeder', '%s QUEUED (message-id=%s from=%s to=%s)', id, envelope.messageId, envelope.from, toList);
