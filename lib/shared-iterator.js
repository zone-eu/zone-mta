...
class SharedIterator {
    constructor(queue, name, options) {
        ...
        console.log('INITIALIZED ITERATOR %s', this.name);
    ...
    getNext(callback) {
        ...
        this.iterator.next((err, key, value) => {
            ...
            if (typeof key === 'undefined') {
                console.log('ITERATOR REACHED END, FOUND %s ITEMS', this.iterator.itemCount);
                console.log(this.options);
                // nothing found, clear the iterator
                this.iterator.end(() => callback(null, false));
                this.iterator = false;
                return;
            }
            ...
            if (!this.nextQueue.length) {
                ...
                this.timeout = setTimeout(() => {
                    if (this.iterator) {
                        this.iterator.end(() => false);
                        this.iterator = false;
                        console.log('REMOVING ITERATOR %s', this.name);
                        console.log(this.options);
