...
const deliveryStatusCounter = new promClient.Counter('zonemta_delivery_status', 'Delivery status', ['result']);
const messagePushCounter = new promClient.Counter('zonemta_message_push', 'Messages pushed to queue', ['result']);
const bounceCounter = new promClient.Counter('zonemta_bounce_generation', 'Bounce generation');
const dropCounter = new promClient.Counter('zonemta_message_drop', 'Messages dropped');
                    ...
                    case 'RELEASE':
                        {
                            ...
                            return this.releaseDelivery(client.zone, client.id, data, (err, response) => {
                                ...
                                plugins.handler.runHooks('queue:release', [client.zone, data], () => client && client.send({
                                    req: data.req,
                                    response
                                }));
                            });
                    ...
                    case 'BOUNCE':
                        {
                            ...
                            plugins.handler.runHooks('queue:bounce', [bounce, this.maildrop], () => client && client.send({
                                req: data.req,
                                response: true
                            }));
                        }
