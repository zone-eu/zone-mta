...
class SMTPProxy {
    ...
    // Tell receiver children to shut down over the fork IPC channel after the proxy has
    // stopped accepting sockets but before the queue server closes. Setting `closing` makes
    // the child 'close' handler above treat their exit as expected instead of respawning.
    closeChildren() {
        this.closing = true;
        this.children.forEach(child => {
