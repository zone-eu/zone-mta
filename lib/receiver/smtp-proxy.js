...
class SMTPProxy {
    ...
    spawnReceiver() {
        if (this.closing || this.children.size >= this.processes) {
            return false;
        }
    ...
    // Tell the receiver children to shut down, see lib/child-shutdown.js. Called after the
    // proxy has stopped accepting sockets. Setting `closing` makes the child 'close' handler
    // above treat their exit as expected instead of respawning.
    closeChildren() {
        this.closing = true;
        childShutdown.notifyChildren(this.children, 'SMTP/' + this.name);
    }
}
