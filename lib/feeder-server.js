    ...
    handleMessage(stream, session, callback) {
        ...
        this.queue.store(dkimStream, (err, id) => {
            ...
            headers.add('Received', 'from ' + (envelope.transhost ? envelope.transhost +
                ' ' : '') + '[' + envelope.origin + '] ' + (session.user ? '(Authenticated sender: ' + session.user + ') ' : '') + 'by ' + hostname + ' with ' + envelope.transtype + ' id ' + id + '; ' + new Date(envelope.time).toUTCString().replace(/GMT/, '+0000'), envelope.dkim ? 1 : 0);
