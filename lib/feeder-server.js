    ...
    handleMessage(stream, session, callback) {
        ...
        message.on('headers', headersObj => {
            ...
            if (!from) {
                ...
                mId = '<' + uuid.v4() + '@' + (envelope.from.substr(envelope.from.lastIndexOf('@') + 1) || hostname) + '>';
                headers.add('From', envelope.from || 'anonymous@' + hostname);
            }

            // Check Date: value. Add if missing or invalid or future date
            let date = headers.getFirst('date');
            let dateVal = new Date(date);
            if (!date || dateVal.toString() === 'Invalid Date' || dateVal > new Date()) {
            ...
            // Check From: value. Needed for potential DKIM signing
        ...
        this.queue.store(dkimStream, (err, id) => {
            ...
            headers.add('Received', 'from ' + (envelope.transhost ? envelope.transhost +
                ' ' : '') + '[' + envelope.origin + '] ' + (envelope.user ? '(Authenticated sender: ' + envelope.user + ') ' : '') + 'by ' + hostname + ' with ' + envelope.transtype + ' id ' + id + '; ' + new Date(envelope.time).toUTCString().replace(/GMT/, '+0000'), envelope.dkim ? 1 : 0);
