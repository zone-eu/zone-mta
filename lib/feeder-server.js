            ...
            this.server = new SMTPServer({
                ...
                onAuth: (auth, session, next) => {
                    if (!auth.username || !auth.password || auth.username.length > 1024 || auth.password.length > 1024) {
                        log.info('Feeder', '%s; Failed auth of "%s" from %s[%s] using %s: %s', session.id, auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method, 'Invalid credentials');
                        return next(new Error('Invalid username or password'));
                    }

                    plugins.handler.runHooks('feeder:auth', [auth, session], err => {
                        if (err) {
                            log.info('Feeder', '%s; Failed auth of "%s" from %s[%s] using %s: %s', session.id, auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method, err.message);
                            return setImmediate(() => callback(err));
                        }

                        log.info('Feeder', '%s; Authenticated "%s" from %s[%s] using %s', session.id, auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method);
