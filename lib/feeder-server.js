    ...
    setup(callback) {
        ...
        getKeyfiles(() => {
            this.server = new SMTPServer({
                ...
                onAuth: (auth, session, next) => {
                    if (!auth.username || !auth.password || auth.username.length > 1024 || auth.password.length > 1024) {
                        log.info('Feeder', 'Failed auth of "%s" from %s[%s] using %s: %s', auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method, 'Invalid credentials');
                        return next(new Error('Invalid username or password'));
                    }

                    plugins.handler.runHooks('feeder:auth', [auth, session], err => {
                        if (err) {
                            log.info('Feeder', 'Failed auth of "%s" from %s[%s] using %s: %s', auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method, err.message);
                            return setImmediate(() => callback(err));
                        }

                        log.info('Feeder', 'Authenticated "%s" from %s[%s] using %s', auth.username, session.clientHostname || session.hostNameAppearsAs, session.remoteAddress, auth.method);
            ...
            this.server.on('connect', data => {
                ...
                log.info('Feeder', 'Connection from %s:%s', hostname, data.remotePort);
            });
