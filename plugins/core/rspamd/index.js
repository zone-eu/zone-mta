...
module.exports.init = function (app, done) {
    ...
    app.addHook('message:queue', (envelope, next) => {
        if (app.config.rejectSpam && envelope.spam && envelope.spam.default && envelope.spam.default.is_spam) {
            ...
            app.logger.info('Feeder', '%s NOQUEUE spam score %s, tests=[%s] (from=%s to=%s)', envelope.id, envelope.spam.default.score, envelope.spam.tests.join(','), envelope.messageId, envelope.from, envelope.to.join(','));
        }
        next();
    });
